...
macOS + ZSH での実行を想定しています。

1. [Homebrew](https://brew.sh/) をインストールしてください（インストール済みの場合は省略可）。

```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
```

2. 依存ライブラリをインストールしてください（インストール済みの場合は省略可）。

```bash
brew install coreutils curl git
```

3. [mise](https://github.com/jdx/mise) をインストールしてください（インストール済みの場合は省略可）。

```bash
curl https://mise.run | sh
echo 'eval "$($HOME/.local/bin/mise activate zsh --shims)"' >> ~/.zshrc
source ~/.zshrc
```
